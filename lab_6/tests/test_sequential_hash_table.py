...
from AOIS.lab_6.model.sequential_hash_table import *
...
def test_load_data(monkeypatch):
    ...
    key_data = SequentialHashTable.load_data('test_data.json')
...
def test_update_item1(monkeypatch):
    hash_table = SequentialHashTable()
    hash_table.insert_new_item("inertia", "the ability to maintain velocity")
...
def test_load_data_file_not_found():
    with patch("builtins.open", side_effect=FileNotFoundError):
        result = SequentialHashTable.load_data("non_existing_file.json")
        assert result == []
...
def test_load_data_json_decode_error():
    ...
    with patch("builtins.open", return_value=mock_file):
        with patch("json.load", side_effect=json.JSONDecodeError("Expecting value", "doc", 0)):
            result = SequentialHashTable.load_data("invalid_file.json")
            assert result == []
