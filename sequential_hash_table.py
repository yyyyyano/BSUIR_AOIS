...
class SequentialHashTable:
    ...
    def __calc_num_value_for_keyword(self, keyword):
        ...
        let1 = keyword[0]
        let2 = keyword[1]
        a_val = SequentialHashTable.__get_unicode_num('A')
        let1_value = SequentialHashTable.__get_unicode_num(let1) - a_val
        let2_value = SequentialHashTable.__get_unicode_num(let2) - a_val
    ...
    def __sequential_collision_solve(self, hash_value):
        result = (hash_value + 1) % self.size
        return result
    ...
    def insert_new_item(self, key, data):
        ...
        while self.table[curr_pos].U == 1 and self.table[curr_pos].D == 0:
            curr_pos = self.__sequential_collision_solve(hash_value)
            step += 1
            if step >= self.size:
                print("Table is full")
                return
